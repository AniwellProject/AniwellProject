...
CREATE TABLE pet_feed_log (
                              ...
                              petId      INT UNSIGNED NOT NULL COMMENT 'FK → pet.id',      -- 펫 ID

                              fedAt      DATETIME NOT NULL DEFAULT NOW() COMMENT '급여 시각',

                              brand      VARCHAR(120) NOT NULL COMMENT '브랜드명',
                              feedType   VARCHAR(20)  NOT NULL COMMENT '급여 형태: dry(건식), wet(습식), treat(간식) 등',

                              source     VARCHAR(20)  NULL DEFAULT 'manual'
             COMMENT '기록 출처: manual(수동), hospital(병원), device(기기) 등',
                              note       VARCHAR(255) NULL COMMENT '비고(메모)',

                              regDate    DATETIME NOT NULL DEFAULT NOW() COMMENT '레코드 생성일',
                              updateDate DATETIME NOT NULL DEFAULT NOW() ON UPDATE NOW()
             COMMENT '레코드 수정일',

    -- 인덱스: 펫별 + 시간순 조회 최적화
                              INDEX idx_pet_time (petId, fedAt),

    -- FK: pet.id 와 연결, 펫 삭제 시 로그도 같이 삭제
                              CONSTRAINT fk_feedlog_pet FOREIGN KEY (petId) REFERENCES pet(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
  COMMENT='펫별 급여 이벤트 로그 (브랜드/타입 기반, 횟수 기록)';
...
SELECT id, title, crewId, boardId, regDate, delStatus
FROM article
ORDER BY id DESC
    LIMIT 5;
...
-- (2) crewId만 조건
SELECT * FROM article WHERE crewId = 3;

-- (3) boardId만 조건
SELECT * FROM article WHERE boardId = 3;

-- (4) 삭제 여부 무시
SELECT * FROM article WHERE crewId = 1 AND boardId = 3;
SELECT id, crewId, boardId, delStatus, title
FROM article
WHERE boardId = 3
ORDER BY id DESC;
...
INSERT INTO article
(regDate, updateDate, memberId, boardId, crewId, title, body, hitCount, delStatus)
VALUES
    (NOW(), NOW(), 1, 1, 1, '테스트 공지 제목', '이건 공지사항 본문입니다.', 0, 0);
